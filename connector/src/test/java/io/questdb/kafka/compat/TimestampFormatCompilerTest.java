...
public class TimestampFormatCompilerTest {
    ...
    private static final DateFormat MILLIS_UTC = compiler.compile(CommonUtils.UTC_PATTERN);

    private String microsToString(long micros) {
        StringSink s = new StringSink();
        MILLIS_UTC.format(micros, defaultLocale, "Z", s);
        return s.toString();
    }

    private static final DateFormat USEC_GREEDY_UTC = compiler.compile("yyyy-MM-ddTHH:mm:ss.U+z");

    private long parseTimestamp(String date) throws NumericException {
        return USEC_GREEDY_UTC.parse(date, defaultLocale);
    }
    ...
    public void testBasicParserCompiler() {
        ...
        assertEquals("2017-04-08T23:11:10.123Z", sink.toString());
    }
    ...
    @Test
    public void testFormatBSTtoMSK() throws io.questdb.client.std.NumericException {
        ...
        long matched = defaultLocale.matchZone(targetTimezoneName, 0, targetTimezoneName.length());
        int tzIndex = io.questdb.client.std.Numbers.decodeLowInt(matched);
        millis += defaultLocale.getZoneRules(tzIndex, RESOLUTION_MICROS).getOffset(millis);
        sink.clear();
        fmt.format(millis, defaultLocale, targetTimezoneName, sink);
        assertEquals("06-04-2017 03:09:30 MSK", sink.toString());
    }
    ...
    public void testGreedyYear() {
        MicrosFormatUtils.updateReferenceYear(Micros.toMicros(1997, CommonUtils.isLeapYear(1997), 1, 1, 0, 0));
        try {
            ...
            MicrosFormatUtils.updateReferenceYear(Micros.toMicros(2015, CommonUtils.isLeapYear(2015), 1, 20, 0, 0));
        }
    }
    ...
    public void testNegativeYear() {
        ...
        assertEquals("BC -2010 Aug", sink.toString());
    }
    ...
    public void testOperationUniqueness() {
        ...
        for (int i = 0, n = MicrosFormatCompiler.getOpCount(); i < n; i++) {
            ...
            assertTrue(codeSet.add(code));
            assertTrue(nameSet.add(name));
        }
    }
    ...
    private void assertFormat(String expected, String pattern, String date, int mic) throws NumericException {
        sink.clear();
        long micros = parseTimestamp(date) + mic;
        get(pattern).format(micros, defaultLocale, "GMT", sink);
        assertEquals(expected.toLowerCase(), sink.toString().toLowerCase());
        sink.clear();
    ...
    private void assertThat(String pattern, String expected, String input, DateLocale locale) throws NumericException {
        ...
        assertEquals(expected, microsToString(format.parse(input, locale)));

        DateFormat compiled = compiler.compile(pattern);
        assertEquals(expected, microsToString(compiled.parse(input, locale)));
    }
    ...
    private void testAgainstJavaReferenceImpl(String pattern) throws ParseException {
        ...
        for (long tsMicros = 0, n = TimeUnit.DAYS.toMicros(1); tsMicros < n; tsMicros += step) {
            ...
            assertEquals(javaFormatted, sink.toString());
            ...
            assertEquals(tsMicros, genericQuestFmt.parse(sink, defaultLocale));
            assertEquals(tsMicros, compiledQuestFmt.parse(sink, defaultLocale));

            // sanity check
            assertEquals(tsMillis, javaFmt.parse(sink.toString()).getTime());
        }
    }
}
